...
public static Instruction? FindInstruction(IList<Instruction> instrs, OpCode opcode, int index) {
    int count = 0;
    foreach (var instr in instrs) {
        if (instr.OpCode == opcode) {
            if (count == index) return instr;
            count++;
        }
    }
    return null;
...
public static MethodDef? FindMethod(ModuleDefMD module, IList<OpCode> pattern) {
    foreach (var type in module.Types) {
        foreach (var method in type.Methods) {
            if (!method.HasBody) continue;
            var instrs = method.Body.Instructions;
            if (instrs.Count < pattern.Count) continue;

            int match = 0;
            for (int i = 0; i < instrs.Count && match < pattern.Count; i++) {
                if (instrs[i].OpCode == pattern[match]) match++;
            }
            if (match == pattern.Count) return method;
        }
    }
    return null;
}

public static MethodDef? GetModuleTypeCctor(ModuleDefMD module) {
    foreach (var type in module.Types) {
        if (type.IsGlobalModuleType)
            return type.FindStaticConstructor();
    }
    return null;
}

public static Resource? GetResource(ModuleDefMD module, string name) {
